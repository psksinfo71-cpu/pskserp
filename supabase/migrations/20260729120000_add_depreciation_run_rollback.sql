-- Allow a depreciation run to be removed atomically and restore asset values.
...
BEGIN
  ...
  -- Removing a non-latest run would require replaying all subsequent runs.
  SELECT EXISTS (
    SELECT 1 FROM public.asset_depreciation_runs r
    WHERE r.status = 'completed'
      AND r.run_at > v_run.run_at
  ) INTO v_has_later_run;
  IF v_has_later_run THEN
    RAISE EXCEPTION 'Delete the latest depreciation run first';
  END IF;

  FOR v_txn IN
    SELECT asset_id, amount
    FROM public.asset_transactions
    WHERE depreciation_run_id = v_run.id
  LOOP
